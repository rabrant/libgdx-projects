...
import com.rabrant.bashnbump.AnimationSet;
import com.rabrant.bashnbump.TextureSheet;
...
public class AnimatedTexture {
	...
	public boolean setCurrentAnimation(String name) {
		...
		for (int i = 0; i < myAnimationSetList.size(); i++) {
			if (myAnimationSetList.get(i).getName().matches(name)) {
	...
	public boolean animate(String name) {
		...
		hasRestarted = myAnimationSetList.get(myCurrentAnimationIndex).animate();
	...
	public boolean animate(String name, int framerate, boolean nil) {
		boolean hasRestarted = false;
		if (myCurrentAnimation != name) {
			for (int i = 0; i < myAnimationSetList.size(); i++) {
				if (myAnimationSetList.get(i).getName().matches(name)) {
		...
		myAnimationSetList.get(myCurrentAnimationIndex).setFrameRateMs(framerate);
		hasRestarted = myAnimationSetList.get(myCurrentAnimationIndex).animate();
		myTextureSheet.setIndex(myAnimationSetList.get(myCurrentAnimationIndex).getFrameIndex());
		return hasRestarted;
	}
